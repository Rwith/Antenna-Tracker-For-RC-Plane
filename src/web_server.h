...
const GRAPH_MAX=120;
const METRICS={
  speed:    {label:'Speed',        unit:'km/h',   color:'#60a5fa'},
  altitude: {label:'Altitude',     unit:'m',      color:'#4ade80',stops:[.65,.30]},
  distance: {label:'Distance',     unit:'m',      color:'#f97316'},
  rssi:     {label:'RSSI',         unit:'dBm',    color:'#f87171'},
  lq:       {label:'Link Quality', unit:'%',      color:'#a78bfa'},
  tilt:     {label:'Tilt Angle',   unit:'\u00B0', color:'#facc15'},
};
const gBuf={speed:[],altitude:[],distance:[],rssi:[],lq:[],tilt:[]};
const gLbl=[];let curMetric='speed';
function hexRgb(h){return[parseInt(h.slice(1,3),16),parseInt(h.slice(3,5),16),parseInt(h.slice(5,7),16)];}
const gradFill=ctx=>{const{ctx:c,chartArea:a}=ctx.chart;if(!a)return'transparent';const[r,g,b]=hexRgb(METRICS[curMetric].color);const s=METRICS[curMetric].stops||[.45,.03];const gr=c.createLinearGradient(0,a.top,0,a.bottom);gr.addColorStop(0,`rgba(${r},${g},${b},${s[0]})`);gr.addColorStop(1,`rgba(${r},${g},${b},${s[1]})`);return gr;};
...
const chart=new Chart(document.getElementById('liveChart'),{
  ...
  data:{labels:gLbl,datasets:[{label:'Speed (km/h)',data:gBuf.speed,
    borderColor:'#60a5fa',backgroundColor:gradFill,
