...
const METRICS={
  ...
  altitude: {label:'Altitude',     unit:'m',      color:'#4ade80'},
...
const gradFill=ctx=>{const{ctx:c,chartArea:a}=ctx.chart;if(!a)return'transparent';const[r,g,b]=hexRgb(METRICS[curMetric].color);const gr=c.createLinearGradient(0,a.top,0,a.bottom);gr.addColorStop(0,`rgba(${r},${g},${b},.45)`);gr.addColorStop(1,`rgba(${r},${g},${b},.03)`);return gr;};
